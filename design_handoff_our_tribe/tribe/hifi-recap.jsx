...
function Recap({ tw, onNav, tasks, onToggle, onOpen, cvars }) {
  const [subtab, setSubtab] = React.useState('summary');
  const ranked = [...MEMBERS].sort((a, b) => b.pts - a.pts);
  const total = MEMBERS.reduce((a, m) => a + m.pts, 0);
  const max = Math.max(...MEMBERS.map((m) => m.pts));
  const goal = 100;
    ...
    <div className="app" style={{ ...cvars, '--accent': tw.accent, '--r': tw.radius + 'px' }}>
      <div className="scroll">
        ...
        <div className="panel">
          ...
          <RecapBars ranked={MEMBERS} max={max} />
        </div>
