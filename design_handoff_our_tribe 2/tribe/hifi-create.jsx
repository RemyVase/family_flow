...
function CreateTask({ tw, onClose, onCreate, cvars }) {
  ...
  const [assign, setAssign] = React.useState('person'); // 'person' | 'rotate' | 'unassigned'
  ...
  const recurring = rep !== 'once';
  // 'à tour de rôle' n'a de sens que pour une tâche récurrente
  const effAssign = (assign === 'rotate' && !recurring) ? 'person' : assign;
  ...
  const submit = () => {
    ...
    onCreate({
      ...
      rep: r.rep || undefined,
      pts, done: false,
      who: effAssign === 'person' ? who : null,
      whoName: effAssign === 'person' ? (who === 'lea' ? 'Léa' : whoObj.name) : undefined,
      rotate: effAssign === 'rotate' || undefined,
    });
  };
  ...
  return (
    <div className="app" style={{ ...cvars, '--accent': accent, '--r': tw.radius + 'px' }}>
      <div className="scroll" style={{ paddingTop: 56, paddingBottom: 100 }}>
        ...
        {/* attribution */}
        <div className="cf-group">
          <label className="cf-label">Attribution</label>
          <div className="cf-chips">
            {[{ id: 'person', label: 'Une personne', ic: 'user' },
              ...(recurring ? [{ id: 'rotate', label: 'À tour de rôle', ic: 'repeat' }] : []),
              { id: 'unassigned', label: 'À répartir', ic: 'users' }].map((o) => {
              const on = effAssign === o.id;
              return (
                <button key={o.id} className={'cf-chip' + (on ? ' on' : '')} onClick={() => setAssign(o.id)}
                  style={on ? { borderColor: accent, background: `color-mix(in oklab, ${accent} 14%, #fff)`, color: `color-mix(in oklab, ${accent} 72%, #000 8%)` } : null}>
                  {React.cloneElement(HIcon[o.ic], { style: { width: 13, height: 13 } })}{o.label}
                </button>
              );
            })}
          </div>
          {effAssign === 'unassigned' && <div className="cf-subhint">La tâche ira dans « À répartir », chacun pourra la prendre.</div>}
          {effAssign === 'rotate' && <div className="cf-subhint">À chaque fois, la tribu choisit qui s’en occupe.</div>}
        </div>
