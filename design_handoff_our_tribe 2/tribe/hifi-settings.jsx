...
/* =========================================================
   NOTIFICATIONS
   ========================================================= */
function Notifications({ tw, cvars, onBack }) {
  const accent = tw.accent;
  ...
  const [applause, setApplause] = React.useState(true);
  const [quiet, setQuiet] = React.useState(true);

  const off = !master;
  const dim = off ? { opacity: .45, pointerEvents: 'none' } : null;

  return (
    <div className="app" style={{ ...cvars, '--accent': accent, '--r': tw.radius + 'px' }}>
      <div className="scroll" style={{ paddingBottom: 108 }}>
        <SetHeader title="Notifications" sub="Choisis ce que la tribu reçoit, et quand." onBack={onBack} />

        {/* master switch */}
        <div className="set-card">
          <div className="set-row">
            <span className="set-ic" style={{ background: 'var(--accent-soft)', color: accent }}>{HIcon.bell}</span>
            <div className="set-txt"><div className="set-l">Autoriser les notifications</div><div className="set-d">{master ? 'Activées sur cet appareil' : 'Tout est en pause'}</div></div>
            <Toggle on={master} onChange={setMaster} accent={accent} />
          </div>
        </div>

        {/* Rappels */}
        <div className="dt-label" style={{ marginTop: 20 }}>Rappels de tâches</div>
        <div className="set-card" style={dim}>
          <div className="set-row">
        ...
        {/* Vie de la tribu */}
        <div className="dt-label" style={{ marginTop: 20 }}>Vie de la tribu</div>
        <div className="set-card" style={dim}>
          <div className="set-row">
            <span className="set-ic" style={{ background: 'var(--m-maman-soft)', color: 'var(--m-maman)' }}>{HIcon.gift}</span>
            <div className="set-txt"><div className="set-l">Points &amp; récompenses</div><div className="set-d">Quand un objectif est débloqué</div></div>
            <Toggle on={applause} onChange={setApplause} accent={accent} />
          </div>
        </div>

        {/* Ne pas déranger */}
        <div className="dt-label" style={{ marginTop: 20 }}>Ne pas déranger</div>
        <div className="set-card" style={dim}>
          <div className="set-row">
            <span className="set-ic" style={{ background: 'var(--dusk-soft)', color: 'var(--dusk)' }}>{HIcon.moon}</span>
            <div className="set-txt"><div className="set-l">Nuit calme</div><div className="set-d">Aucune notif de 21:00 à 7:00</div></div>
...
function Reminders({ tw, cvars, onBack }) {
  ...
  return (
    <div className="app" style={{ ...cvars, '--accent': accent, '--r': tw.radius + 'px' }}>
      <div className="scroll" style={{ paddingBottom: 108 }}>
        <SetHeader title="Récurrences" sub="Comment les tâches répétitives et les points se renouvellent." onBack={onBack} />

        <div className="dt-label">Récurrences</div>
        <div className="set-card">
          <div className="set-block">
        ...
        <div className="set-note">{React.cloneElement(HIcon.bell, { style: { width: 14, height: 14, stroke: accent, fill: 'none' } })}<span>Les rappels et notifications se règlent dans « Notifications ».</span></div>
      </div>
