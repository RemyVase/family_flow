...
## Fonctionnalités ajoutées (dernière itération)
Ces comportements sont présents dans les fichiers `tribe/` et pilotés par l'état de `Our Tribe.html` :

- **Attribution des tâches** (création, `tribe/hifi-create.jsx`) : bloc « Attribution » = `Une personne` / `À tour de rôle` (uniquement si récurrente) / `À répartir`. Le picker de personne n'apparaît que pour « Une personne ». Sortie : `who` (id ou `null`), `whoName`, et `rotate: true` pour le tour de rôle.
- **« À répartir »** (accueil + vue dédiée, `tribe/hifi-home.jsx`) : les tâches sans responsable (`!who`) ne s'affichent pas dans les sections par moment ; l'accueil montre un **bandeau** « N tâches à répartir » qui ouvre l'écran `Unassigned` (state `showUnassigned` dans l'App). Chaque carte y propose une rangée d'avatars → `onAssign(id, memberId)` attribue la tâche (elle rejoint alors sa section). Les récurrentes « à tour de rôle » y portent un badge et « Qui s'en occupe cette fois ? ».
- **Rôle & gestion des membres** (`tribe/hifi-profile.jsx`) : prop `isChef` (démo via la tweak « Mon rôle » Chef/Membre ; `CURRENT_USER = 'lea'`). 
  - **Chef** : peut changer la couleur de n'importe quel membre (tap → palette) et **retirer** un membre — `MemberRow` est swipe-to-remove (drag pointer, seuil −44px) avec confirmation inline ; on ne peut pas se retirer soi-même. `onRemoveMember(id)` retire le membre de l'état App `members`, ce qui se répercute dans le Récap (prop `members`).
  - **Membre** : ne peut éditer que **sa** couleur (les autres lignes sont statiques, sans swipe).
- **Quitter la tribu** (`LeaveSheet`, `tribe/hifi-profile.jsx`) : bouton « Quitter la tribu » → bottom-sheet. Un membre part après confirmation. **Le chef doit d'abord désigner un nouveau chef** (avertissement + sélecteur des autres membres, bouton « Nommer & quitter » désactivé tant qu'aucun choix). Cas limite : dernier membre → « Supprimer la tribu ».
- **Édition de profil** (`ProfileEdit`) : avatar + prénom + e-mail + « Ma couleur » (change partout via `onSetColor`) + « Changer le mot de passe ». Ouvert depuis « Éditer ».
- **Notifications** (`Notifications`, `tribe/hifi-settings.jsx`) : écran dédié — interrupteur maître (grise le reste si off), rappels de tâches (+ délai), récapitulatifs (matin/soir/retard), vie de la tribu, « Nuit calme ». Séparé des **Récurrences** (`Reminders` : début de semaine, report auto).
...
- `tribe/hifi-home.jsx` — Accueil (+ définit l'objet global `HIcon`, réutilisé par les autres écrans ; contient aussi `UnassignedCard` et l'écran `Unassigned`).
- `tribe/hifi-recap.jsx` — Récap / Classement (accepte une prop `members` pour refléter les départs).
- `tribe/hifi-create.jsx` — Création de tâche (avec attribution : personne / à tour de rôle / à répartir).
- `tribe/hifi-profile.jsx` — Profil (+ `MemberRow` swipe-to-remove, `LeaveSheet` quitter la tribu, `ProfileEdit` édition).
- `tribe/hifi-settings.jsx` — Sous-écrans Réglages (`Rewards`, `Reminders` = Récurrences, `Notifications`).
- `tribe/hifi-onboarding.jsx` — Onboarding.
- `tribe/hifi-week.jsx` — vue Semaine (fork tribe : en-tête avec sélecteur de semaine).
- `hifi-detail.jsx`, `hifi-tasks.jsx` — écrans partagés (style-agnostiques, rendus par le CSS tribe).
